...
app.get("/Item/:Type_ID/:School_ID/:Obj", function (req, resp) {
  connection.query(
    `IF ((${req.params.Type_ID} != 'x') AND (${req.params.School_ID} = 'x'))
    BEGIN
    ...
    END
    ELSE IF ((${req.params.Type_ID} = 'x') AND (${req.params.School_ID} != 'x'))
    BEGIN
        SELECT * FROM ItemDonate WHERE 
        Quantity!=0 AND Warning!=5 AND Obj LIKE '%${req.params.Obj}%'
        AND School_ID =${req.params.School_ID} AND State=1 ORDER BY Item_ID DESC
    END
    ELSE
    BEGIN
        SELECT * FROM ItemDonate WHERE 
