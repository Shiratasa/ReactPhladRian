...
app.post("/N_check/:Student_ID", function (req, resp) {
  var Student_ID = req.body.Item_ID;
  connection.query(
    `SELECT * FROM WebNotify WHERE Student_ID = ${req.params.Student_ID} AND State=1`,
    function (error, result) {
      if (error) {
        console.error("Query failed:\n" + error.stack);
        connection.end();
...
app.post("/R_regis", upload.any(), function (req, resp) {
  ...
  connection.query(
    "INSERT INTO UserStudent (Username, Email, Phone, Address, Student_Card, School_ID, Grade, Card_Image, Password, ZIP, Chance, State) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 5, 0)",
    [
      Username,
      Email,
      Phone,
...
app.post("/D_regis", function (req, resp) {
  ...
  connection.query(
    "INSERT INTO UserDonor (Username, Email, Phone, Address, Password, ZIP, Point, State, Death) VALUES (?, ?, ?, ?, ?, ?, 0, 0, 0)",
...
app.get("/Notify/:Student_ID", function (req, resp) {
  connection.query(
    `SELECT * FROM WebNotify WHERE Student_ID=${req.params.Student_ID}`,
    function (error, result) {
      if (error) {
        console.error("Query failed:\n" + error.stack);
        connection.end();
...
app.get("/:Student_ID/Notify2/:School_ID", function (req, resp) {
  connection.query(
    `SELECT * FROM ItemDonate JOIN WebNotify ON ItemDonate.School_ID = WebNotify.School_ID 
    WHERE ItemDonate.School_ID=${req.params.School_ID} AND WebNotify.Student_ID=${req.params.Student_ID} 
    AND WebNotify.State=1 ORDER BY Item_ID DESC LIMIT 7`,
    function (error, result) {
      if (error) {
        console.error("Query failed:\n" + error.stack);
        connection.end();
...
app.post("/NotiBell", function (req, resp) {
  var School_ID = req.body.School_ID;
  var Student_ID = req.body.Student_ID;
  var State = req.body.State;
  connection.query(
    "INSERT INTO WebNotify (School_ID, Student_ID, State) VALUES (?, ?, 1)",
    [School_ID, Student_ID, State],
    function (error, result) {
      if (error) {
        console.error("Insert failed:\n" + error.stack);
        connection.end();
...
app.post("/NotiBellDel", function (req, resp) {
  var School_ID = req.body.School_ID;
  var Student_ID = req.body.Student_ID;
  connection.query(
    `UPDATE WebNotify SET State=0 WHERE Student_ID=${Student_ID} AND School_ID=${School_ID}`,
    function (error, result) {
      if (error) {
        console.error("Insert failed:\n" + error.stack);
        connection.end();
...
app.post("/R_mailAu", function (req, resp) {
  ...
  var mailOptions = {
    ...
    to: `${Email}`,
    subject: "Email authentication for PhladRian Receiver account",
...
app.post("/D_mailAu", function (req, resp) {
  ...
  var mailOptions = {
    from: "phladriansystem@gmail.com",
    to: `${Email}`,
    subject: "Email authentication for PhladRian Donor account",
