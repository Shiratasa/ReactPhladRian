...
app.post("/Notify2", function (req, resp) {
  var School_ID = req.body.School_ID;
  var Type_ID = req.body.Type_ID;
  connection.query(
    `SELECT * FROM ItemDonate WHERE School_ID=${School_ID} OR Type_ID=${Type_ID}`,
    function (error, result) {
      if (error) {
        console.error("Query failed:\n" + error.stack);
        connection.end();
        ...
        console.log(result);
        TypeValue = Type_ID;
        SchoolValue = School_ID;
      }
    }
  );
});
app.get("/Notify3", function (req, resp) {
  connection.query(
    `SELECT * FROM ItemDonate WHERE School_ID=? OR Type_ID=?`,[SchoolValue, TypeValue],
    function (error, result) {
      if (error) {
        console.error("Query failed:\n" + error.stack);
        connection.end();
