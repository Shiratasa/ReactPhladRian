...
app.get("/Grade", function (req, resp) {
  connection.query("SELECT * FROM AllGrade WHERE Grade!=13", function (error, result) {
    if (error) {
      console.error("Query failed:\n" + error.stack);
      connection.end();
      throw error;
...
app.post("/Trade_D", function (req, resp) {
  var Donor_ID = req.body.Student_ID;
  var Reward_ID = req.body.Item_ID;
  var GetItem = req.body.GetItem;
  var Accept = req.body.Accept;
  var State = req.body.State;
  connection.query(
    "INSERT INTO WebRequest (Donor_ID, Reward_ID, GetItem, Accept, State) VALUES (?, ?, 0, 0, 1)",
    [Donor_ID, Reward_ID, GetItem, Accept, State],
    function (error, result) {
      if (error) {
        console.error("Insert failed:\n" + error.stack);
        connection.end();
