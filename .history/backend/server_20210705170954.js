...
app.get("/QR_detail/:Request_ID", function (req, resp) {
  connection.query(
    `SELECT * FROM WebRequest 
     ...
     WHERE Request_ID = ${req.params.Request_ID} AND WebRequest.State=1`,
...
app.get("/WishExist/:Student_ID", function (req, resp) {
  connection.query(
    `SELECT * FROM ItemDonate 
    JOIN WebWishlist ON ItemDonate.Item_ID = WebWishlist.Item_ID 
    WHERE ItemDonate.Quantity!=0 AND ItemDonate.Warning!=5 
    AND ItemDonate.State=1 AND WebWishlist.Student_ID = ${req.params.Student_ID} 
    ORDER BY ItemDonate.Item_ID DESC`,
    function (error, result) {
      if (error) {
        console.error("Query failed:\n" + error.stack);
        connection.end();
...
app.post("/Wish_R", function (req, resp) {
  ...
  var State = req.body.State;
  connection.query(
    "INSERT INTO WebWishlist (Student_ID, Item_ID, Track_ID, State) VALUES (?, ?, ?, 1)",
    [Student_ID, Item_ID, Track_ID, State],
    function (error, result) {
      if (error) {
        console.error("Insert failed:\n" + error.stack);
        connection.end();
