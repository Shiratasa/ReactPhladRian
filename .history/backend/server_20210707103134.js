...
app.get("/Item/:Obj", function (req, resp) {
  connection.query(
    `SELECT * FROM ItemDonate WHERE 
    Quantity!=0 AND Warning!=5 AND Obj LIKE '%${req.params.Obj}%' 
    AND State=1 ORDER BY Item_ID DESC`,
    function (error, result) {
      if (error) {
        console.error("Query failed:\n" + error.stack);
        connection.end();
...
app.get("/Item/:Type_ID", function (req, resp) {
  connection.query(
    `SELECT * FROM ItemDonate WHERE 
    Quantity!=0 AND Warning!=5 AND Type_ID=${req.params.Type_ID}
    AND State=1 ORDER BY Item_ID DESC`,
    function (error, result) {
      if (error) {
        console.error("Query failed:\n" + error.stack);
...
app.get("/Item/:Type_ID/:Obj", function (req, resp) {
  connection.query(
    `SELECT * FROM ItemDonate WHERE 
    Quantity!=0 AND Warning!=5 AND Type_ID=${req.params.Type_ID} 
    AND Obj LIKE '%${req.params.Obj}%' AND State=1 ORDER BY Item_ID DESC`,
    function (error, result) {
      if (error) {
        console.error("Query failed:\n" + error.stack);
        connection.end();
...
app.get("/Item/:School_ID", function (req, resp) {
  connection.query(
    `SELECT * FROM ItemDonate WHERE 
    Quantity!=0 AND Warning!=5 AND School_ID=${req.params.School_ID}
    AND State=1 ORDER BY Item_ID DESC`,
    function (error, result) {
      if (error) {
        console.error("Query failed:\n" + error.stack);
...
app.get("/Item/:School_ID/:Obj", function (req, resp) {
  connection.query(
    `SELECT * FROM ItemDonate WHERE 
    Quantity!=0 AND Warning!=5 AND School_ID=${req.params.School_ID} 
    AND Obj LIKE '%${req.params.Obj}%' AND State=1 ORDER BY Item_ID DESC`,
...
app.get("/Item/:Type_ID/:School_ID", function (req, resp) {
  connection.query(
    `SELECT * FROM ItemDonate WHERE 
    Quantity!=0 AND Warning!=5 AND School_ID=${req.params.School_ID} 
    AND Type_ID=${req.params.Type_ID} AND State=1 ORDER BY Item_ID DESC`,
    function (error, result) {
      if (error) {
        console.error("Query failed:\n" + error.stack);
        connection.end();
...
app.get("/Item/:Type_ID/:School_ID/:Obj", function (req, resp) {
  connection.query(
    `SELECT * FROM ItemDonate WHERE 
    Quantity!=0 AND Warning!=5 AND School_ID=${req.params.School_ID} 
    AND Type_ID=${req.params.Type_ID} AND Obj LIKE '%${req.params.Obj}%' 
    AND State=1 ORDER BY Item_ID DESC`,
    function (error, result) {
      if (error) {
        console.error("Query failed:\n" + error.stack);
...
app.post("/Report", function (req, resp) {
  ...
  var State = req.body.State;
  connection.query(
    "INSERT INTO WebReport (Topic, Email, Message, View, State) VALUES (?, ?, ?, 0, 1)",
    [Topic, Email, Message, View, State],
    function (error, result) {
      if (error) {
        console.error("Insert failed:\n" + error.stack);
        connection.end();
