...
app.post("/S_warning/:Reward_ID", upload.any(), function (req, resp) {
  connection.query(
    `UPDATE ItemReward SET Warning = Warning + 1 WHERE Item_ID = ${req.params.Reward_ID}`,
    function (error, result) {
      if (error) {
        console.error("Insert failed:\n" + error.stack);
        connection.end();
...
app.post("/Report", function (req, resp) {
  ...
  var State = req.body.State;
  connection.query(
    "INSERT INTO WebReport (Student_ID, Donor_ID, Sponsor_ID, Topic, Email, Message, View, State) VALUES (?, ?, ?, ?, ?, ?, 0, 1)",
    [Student_ID, Donor_ID, Sponsor_ID, Topic, Email, Message, View, State],
    function (error, result) {
      if (error) {
        console.error("Insert failed:\n" + error.stack);
        connection.end();
