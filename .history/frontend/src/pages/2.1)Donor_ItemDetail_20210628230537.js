...
function D_Add() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (invali1 == "0" && invali2 == "1" && invali3 == "1") {
    ...
    if (invali1 == "1" && invali2 == "0" && invali3 == "1") {
      document.querySelector("del_but").addEventListener("click", function () {
        JSAlert.confirm("Are you sure you want to delete this item?").then(
          function (result) {
            if (!result) {
              ...
              invali2 = "0";
              invali3 = "0";
              JSAlert.alert("Item deleted!");
            }
          }
        );
      });
      if (invali1 == "1" && invali2 == "0" && invali3 == "0") {
        await timeout(1000).then($(this).unbind("submit").submit());
        history.push(`/d_main/${Donor_ID}`);
        location.reload();
      }
  ...
  const I_delete = async () => {
    invali2 = "0";
    Axios.post(`http://localhost:5000/I_delete/${Item_ID}`, {}).then(
      (response) => {
        console.log(response);
      }
    );
