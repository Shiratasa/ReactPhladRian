...
function R_Main() {
  ...
  const NotiBell = async () => {
    JSAlert.confirm("Do you want to get notification when your school items arrive?").then(
      function (result) {
        if (!result) {
          throw "exit";
        }
  ...
  const N_butC = async () => {
    Axios.post(`http://localhost:5000/N_check/${Student_ID}`, {
    }).then((response) => {
      if (response.data.message) {
        NotiBell();
      } else {
        JSAlert.confirm("You already enable the notification, Do you want to disable it?").then(
          function (result) {
            if (!result) {
              throw "exit";
            }
                              ...
                              <div className="table-responsive cart_info">
                                <table className="table table-condensed">
                                  <thead className="framePic">
                                    ...
                                    <a
                                      class="btn add-to-cart floatBut2"
