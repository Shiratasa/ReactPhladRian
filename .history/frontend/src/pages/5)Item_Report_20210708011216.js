...
function I_Report() {
  ...
  const [Topic_Reg, setTopic_Reg, setTopic_Ref] = useStateRef("");
  const [Mail_Reg, setMail_Reg, setMail_Ref] = useStateRef("");
  ...
  const D_Warn = async () => {
    Axios.post(`http://localhost:5000/D_warning/${Item_ID}`, {}).then(
      (response) => {
  ...
  const ReportAM = async () => {
    setTopic_Reg(
      "Item_ID: " + R_Param1 + " problem. Reported by Student_ID " + Reporter
    );
    setMail_Reg(Link);
    checkStringAM();
    JSAlert.alert(
    ...
    JSAlert.confirm("Do you REALLY want to report this item?").then(function (
      ...
      Axios.post("http://localhost:5000/Report", {
        Student_ID: Reporter,
        Topic: setTopic_Ref.current,
        Email: setMail_Ref.current,
        Message: Message_Reg,
      });
                                  ...
                                  <fieldset>
                                    <input
                                      ...
                                      value={
                                        "Item_ID: " +
                                        R_Param1 +
                                        " problem. Reported by Student_ID " +
                                        Reporter
                                      }
                                      disabled
                                    />
