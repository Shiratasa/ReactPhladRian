...
function R_Main() {
  ...
  const ItemBlock = async () => {
    Axios.get(`http://localhost:5000/Item/r_main/${Student_ID}/${Type_ID}/${School_ID}/${Obj}`).then((response) => {
  ...
  function filterRedirection() {
    history.push(
      `/r_main/${Student_ID}/${textQuery.value}/${categQuery.value}/${schoolQuery.value}`
    );
    window.location.reload();
  }
