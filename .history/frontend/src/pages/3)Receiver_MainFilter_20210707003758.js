...
function R_Main() {
  ...
  const [Search_List, setSearch_List, Search_ListRef] = useStateRef([]);
  ...
  const ItemBlock = async () => {
    Axios.get(`http://localhost:5000/Item/${Obj}`).then((response) => {
  ...
  function filterRedirection() {
    if (
      Search_ListRef.Current == "" &&
    ...
    if (
      ...
      history.push(`/r_main/${Student_ID}/${Search_ListRef.Current}`);
      window.location.reload();
    }
  }
