...
function Authen() {
  ...
  const handleSubmit1 = async (e) => {
    e.preventDefault();
    if (invali == "0") {
      JSAlert.alert("", "Confirm your email to complete this account creation!", JSAlert.Icons.Success);
  ...
  const R_authenMail = async () => {
    Axios.post("http://localhost:5000/R_mailAu", {
      Email: R_emailReg,
    });
  };
  const D_authenMail = async () => {
    Axios.post("http://localhost:5000/D_mailAu", {
      Email: D_emailReg,
    });
  };
