...
function D_Main() {
  ...
  const [R_AddrReg, setR_AddrReg, setR_AddrRef] = useStateRef("");
  const [D_AddrReg, setD_AddrReg, setD_AddrRef] = useStateRef("");
  ...
  function DistanceCal() {
    if (
      setR_AddrRef.current - setD_AddrRef > 300 &&
      setR_AddrRef.current - setD_AddrRef < -300
    ) {
      alert(setR_AddrRef.current - setD_AddrRef);
    }
  }
